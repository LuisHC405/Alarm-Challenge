...
export class PostgresAlarmRepository implements AlarmRepository {
  ...
  async list(): Promise<AlarmRecord[]> {
    ...
    const result = await this.pool.query<AlarmRow>(
      'SELECT id, name, time, challenge_type, difficulty, enabled, created_at, updated_at FROM alarms ORDER BY time ASC, created_at ASC',
    );

    return result.rows.map(mapAlarmRow);
  }

  async findById(id: string): Promise<AlarmRecord | null> {
    await this.ensureSchema();
    const result = await this.pool.query<AlarmRow>(
      'SELECT id, name, time, challenge_type, difficulty, enabled, created_at, updated_at FROM alarms WHERE id = $1',
      [id],
    );

    return result.rows[0] ? mapAlarmRow(result.rows[0]) : null;
  }

  async create(input: AlarmInput): Promise<AlarmRecord> {
    await this.ensureSchema();
    const result = await this.pool.query<AlarmRow>(
      `INSERT INTO alarms (id, name, time, challenge_type, difficulty, enabled)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id, name, time, challenge_type, difficulty, enabled, created_at, updated_at`,
      [
        randomUUID(),
        input.name?.trim() || 'Alarme',
  ...
  async update(id: string, input: Partial<AlarmInput>): Promise<AlarmRecord | null> {
    ...
    const result = await this.pool.query<AlarmRow>(
      ...
      [
        id,
        input.name === undefined ? current.name : input.name.trim() || current.name,
        input.time === undefined ? current.time : normalizeTime(input.time),
        input.challengeType === undefined ? current.challengeType : normalizeChallengeType(input.challengeType),
        input.difficulty === undefined ? current.difficulty : normalizeDifficulty(input.difficulty),
...
function mapAlarmRow(row: AlarmRow): AlarmRecord {
  return {
    id: row.id,
    name: row.name,
    time: row.time,
