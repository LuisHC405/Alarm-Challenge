...
import { normalizeChallengeType, normalizeDifficulty, normalizeTime, type AlarmRepository } from './alarm.repository';
...
export class PostgresAlarmRepository implements AlarmRepository {
  ...
  async list(): Promise<AlarmRecord[]> {
    ...
    const result = await this.pool.query<AlarmRow>('SELECT id, name, time, challenge_type, difficulty, enabled, created_at, updated_at FROM alarms ORDER BY time ASC, created_at ASC');
    return result.rows.map(mapRow);
  }

  async findById(id: string): Promise<AlarmRecord | null> {
    await this.ensureSchema();
    const result = await this.pool.query<AlarmRow>('SELECT id, name, time, challenge_type, difficulty, enabled, created_at, updated_at FROM alarms WHERE id = $1', [id]);
  ...
  async create(input: AlarmInput): Promise<AlarmRecord> {
    ...
    const result = await this.pool.query<AlarmRow>(
      'INSERT INTO alarms (id, name, time, challenge_type, difficulty, enabled) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, name, time, challenge_type, difficulty, enabled, created_at, updated_at',
      [randomUUID(), input.name?.trim() || 'Alarme', normalizeTime(input.time), normalizeChallengeType(input.challengeType), normalizeDifficulty(input.difficulty), input.enabled ?? true],
    );
  ...
  async update(id: string, input: Partial<AlarmInput>): Promise<AlarmRecord | null> {
    ...
    const result = await this.pool.query<AlarmRow>(
      'UPDATE alarms SET name = $2, time = $3, challenge_type = $4, difficulty = $5, enabled = $6, updated_at = NOW() WHERE id = $1 RETURNING id, name, time, challenge_type, difficulty, enabled, created_at, updated_at',
...
function mapRow(row: AlarmRow): AlarmRecord {
  return { id: row.id, name: row.name, time: row.time, challengeType: row.challenge_type, difficulty: row.difficulty, enabled: row.enabled, createdAt: row.created_at.toISOString(), updatedAt: row.updated_at.toISOString() };
}
